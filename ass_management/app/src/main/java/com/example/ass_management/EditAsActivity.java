 ...
 */
public class EditAsActivity extends AppCompatActivity {
    ...
    private void okhttpbuild(BuildAss buildAss){
        try {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(EditAsActivity.this,"申请成功",Toast.LENGTH_SHORT).show();
                }
            });
            finish();
        }catch (Exception e){
...
//            System.out.println(e);
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(EditAsActivity.this,"网络异常", Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
    private void click() {
        btn_cancel.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Intent intent = new Intent(EditAsActivity.this,ProgressSecondActivity.class);
                ProgressSecondActivity.mActivity.finish();
    ...
    private void okhttpdel() {
        try {
            ...
            OkHttpClient client = new OkHttpClient();
            Request request = new Request.Builder()
                    .url(ip + "/delBuildAss?build_id="+staticBuildAss.getBuild_id())
                    .get()
                    .build();
            client.newCall(request).execute();
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(EditAsActivity.this,"删除成功",Toast.LENGTH_SHORT).show();
                }
            });
        }catch (Exception e){
            //其他注册失败异常，比如网络未连接或其他问题
...
//                progressDialog.dismiss();//前面点击登录按钮时出现进度条，这里登录失败进度条消失
                //在登录界面生成对话框提示
                new AlertDialog.Builder(EditAsActivity.this)
                        .setTitle("错误")
                        .setTitle(title)
                        .setCancelable(true)
