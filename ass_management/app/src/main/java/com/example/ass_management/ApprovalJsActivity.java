 ...
 */
public class ApprovalJsActivity extends AppCompatActivity {
    ...
    private void okhttpjoin(JoinAss joinAss){
        try {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(ApprovalJsActivity.this,"修改成功",Toast.LENGTH_SHORT).show();
                }
            });
        }catch (Exception e){
...
//            System.out.println(e);
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(ApprovalJsActivity.this,"网络异常", Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
    private void click() {
        btn_cancel.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Intent intent = new Intent(ApprovalJsActivity.this,JoinAssApprovalActivity.class);
                startActivity(intent);
                JoinAssApprovalActivity.mActivity.finish();
                finish();
            }
        });
...
//                    System.out.println(joinAss.toString());
                    new Thread(){
                        @Override
                        public void run() {
                            //继续注册
    ...
    private void okhttpreject() {
        try {
            ...
            OkHttpClient client = new OkHttpClient();
            Request request = new Request.Builder()
                    .url(ip + "/rejectJoinAss?join_id="+staticJoinAss.getJoin_id())
                    .get()
                    .build();
            client.newCall(request).execute();
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(ApprovalJsActivity.this,"处理成功",Toast.LENGTH_SHORT).show();
                }
            });
        }catch (Exception e){
            //其他注册失败异常，比如网络未连接或其他问题
...
//                progressDialog.dismiss();//前面点击登录按钮时出现进度条，这里登录失败进度条消失
                //在登录界面生成对话框提示
                new AlertDialog.Builder(ApprovalJsActivity.this)
                        .setTitle("错误")
                        .setTitle(title)
                        .setCancelable(true)
